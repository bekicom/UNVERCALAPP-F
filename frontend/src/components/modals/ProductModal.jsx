...
export function ProductModal({
  ...
  const totalPurchaseCost = (Number(form.purchasePrice) || 0) * (Number(form.quantity) || 0);
  const debtAmount = Math.max(0, totalPurchaseCost - (Number(form.paidAmount) || 0));

  return (
    <div className="modal-backdrop" onClick={onClose}>
      <div className="modal-card" onClick={(e) => e.stopPropagation()}>
        <h3>{form.id ? "Mahsulotni tahrirlash" : "Yangi mahsulot yaratish"}</h3>
        <form className="modal-form" onSubmit={onSubmit}>
            ...
            <select
              ...
              onChange={(e) =>
                setForm((p) => ({
                  ...p,
                  unit: e.target.value,
                  ...(e.target.value === "qop" ? {} : { allowPieceSale: false, pieceUnit: "kg", pieceQtyPerBase: "", piecePrice: "" })
                }))
              }
            >
              {PRODUCT_UNITS.map((u) => (
          ...
          <button type="button" className="ghost" onClick={openCreateSupplierModal}>+ Yangi yetkazib beruvchi</button>
          <label>
            Miqdori
          ...
          <label>
            {form.unit === "qop" ? "Qop narxi (chakana)" : "Dona narxi (chakana)"}
            ...
            <select
              ...
              onChange={(e) =>
                setForm((p) => ({
                  ...
                  paidAmount: e.target.value === "naqd" ? String(totalPurchaseCost) : e.target.value === "qarz" ? "0" : p.paidAmount
                }))
              }
            >
          ...
          {form.paymentType === "qisman" ? (
            <label>
              ...
              <input type="number" min="0" max={totalPurchaseCost} step="0.01" value={form.paidAmount} onChange={(e) => setForm((p) => ({ ...p, paidAmount: e.target.value }))} required />
            </label>
          ) : null}
          <p className="hint">Umumiy kelish summasi: {formatMoney(totalPurchaseCost)} so'm</p>
          <p className="hint">Joriy qarz: {formatMoney(debtAmount)} so'm</p>
          {form.unit === "qop" ? (
            <div className="piece-box">
              <label className="check-line">
                <input type="checkbox" checked={form.allowPieceSale} onChange={(e) => setForm((p) => ({ ...p, allowPieceSale: e.target.checked }))} />
              ...
              {form.allowPieceSale ? (
                <>
                  <label>
                    ...
                    <input type="number" min="0" step="0.01" value={form.pieceQtyPerBase} onChange={(e) => setForm((p) => ({ ...p, pieceQtyPerBase: e.target.value }))} required />
                  </label>
                  <p className="hint">Tavsiya (qop narxidan): {suggestedPiecePrice || 0} so'm/{form.pieceUnit}</p>
                  <label>
          ...
          <div className="modal-actions">
            ...
            <button type="submit" disabled={loading || categories.length === 0 || suppliers.length === 0}>{loading ? "Saqlanmoqda..." : "Saqlash"}</button>
          </div>
        </form>
      </div>
    </div>
